...
class HeroesSpinnerAdapter(
    context: Context,
    private val values: List<Hero>,
...
): ArrayAdapter<Hero>(context, listItemRes, values) {

    override fun getCount(): Int = values.size + if (withHeader) 1 else 0
    override fun getItem(position: Int): Hero? =
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        view.text = getItem(position)?.getName() ?: headerText
        return view
    }
    ...
    interface HeroSelectedListener {
        fun onHeroSelected(hero: Hero?, humanType: HumanType)
    }
}