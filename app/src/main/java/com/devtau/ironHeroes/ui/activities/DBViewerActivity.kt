...
import com.devtau.ironHeroes.data.source.local.SQLHelper
import com.devtau.ironHeroes.util.Logger
import com.devtau.ironHeroes.util.toast
import java.util.*
/**
 * Messy activity stolen from somewhere that gives pretty good ui runtime access to app database
 */
...
class DBViewerActivity: AppCompatActivity() {
            ...
            override fun onItemSelected(parent: AdapterView<*>, view: View, pos: Int, id: Long) {
                ...
                if (pos != 0) {
                    ...
                    if (cursor != null) {
                        ...
                        spinnerTable.onItemSelectedListener = object: OnItemSelectedListener {
                            override fun onItemSelected(parentView: AdapterView<*>, selectedItemView: View, position: Int, id: Long) {
                                ...
                                if (spinnerTable.selectedItem.toString() == "Add row to this table") {
                                    ...
                                    if (isEmpty) {
                                        ...
                                        for (next in addNewRowNames) addNewRowValues.add(EditText(this@DBViewerActivity))
                                    } else {
                                        ...
                                        if (!isFinishing) {
                                            ...
                                            AlertDialog.Builder(this@DBViewerActivity)
                                                    ...
                                                    .setPositiveButton("Add") { _, _ ->
                                                        ...
                                                        for ((i, next) in addNewRowNames.withIndex()) {
                                                            query = if (i == addNewRowNames.size - 1)
                                                                query + next.text.toString()
                                                            else
                                                                query + next.text.toString() + ", "
                                                        }
                                                        query = "$query ) VALUES ( "
                ...
                AlertDialog.Builder(this)
                        ...
                        .setPositiveButton("Ok") { _, _ ->
                            ...
                            if (spinnerValue.equals("Update this row", ignoreCase = true)) {
                                ...
                                query = "$query where "
                                for ((i, next) in columnNames.withIndex()) {
                                    if (valueStringLocal?.get(i) != "null") {
                                        query = query + next.text.toString() + " = "
                                        query = if (i == columnNames.size - 1)
    ...
    private fun getData(Query: String): ArrayList<Cursor?> {
        ...
        try {
            ...
            e.message?.let { Logger.d("printing exception", it) }
